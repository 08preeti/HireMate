...
export default function JobDetails() {
  ...
  const handleApply = async () => {
    ...
    try {
      ...
      toast(t.appliedSuccess, "success");
      setShowApply(false);
      setAppliedSuccess(true);
    } catch { toast("Error submitting application", "error"); }
    setSubmitting(false);
  };
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.5)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 9999, padding: 20 }}>
          <div style={{ background: "#fff", borderRadius: 20, padding: 32, width: "100%", maxWidth: 380, textAlign: "center" }}>
            ...
            <div style={{ fontSize: 22, fontWeight: 800, color: "#10b981", marginBottom: 8 }}>
              {language === "hi" ? "आवेदन भेज दिया!" : language === "mr" ? "अर्ज पाठवला!" : "Application Sent!"}
            </div>
            <div style={{ fontSize: 14, color: "#6b7280", marginBottom: 8, lineHeight: 1.6 }}>
              {language === "hi" ? `${job.companyName || "नियोक्ता"} को आपका आवेदन मिल गया है।` : language === "mr" ? `${job.companyName || "नियोक्ता"} ला तुमचा अर्ज मिळाला आहे.` : `Your application has been sent to ${job.companyName || "the employer"}.`}
            </div>
            <div style={{ fontSize: 13, color: "#006491", fontWeight: 600, marginBottom: 24, background: "#e8f4f8", borderRadius: 10, padding: "10px 14px" }}>
              📞 {language === "hi" ? "नियोक्ता जल्द कॉल करेगा" : language === "mr" ? "नियोक्ता लवकरच कॉल करेल" : "The employer will call you soon"}
            </div>
            <button onClick={() => setAppliedSuccess(false)}
              ...
              {language === "hi" ? "ठीक है 👍" : language === "mr" ? "ठीक आहे 👍" : "Got it 👍"}
            </button>
          </div>
        </div>
      )}

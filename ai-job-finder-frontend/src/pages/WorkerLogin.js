...
export default function WorkerLogin() {
  ...
  const sendOTP = async () => {
    ...
    try {
      ...
      if (data.otp) setDevOtp(data.otp); // always show for demo
      setStep(STEPS.OTP);
...
export default function WorkerLogin() {
  ...
  // Step 3 — Complete profile for new worker
  // Use /register directly to avoid OTP expiry issues on profile setup
  const completeProfile = async () => {
    if (!name.trim()) { setError("Please enter your name"); return; }
    if (!selectedSkill) { setError("Please select your skill"); return; }
    setError(""); setLoading(true);
    try {
      // First try verify-otp with profile data
      const res = await fetch(`${BASE}/api/workers/verify-otp`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ phone, otp, name, skills: selectedSkill, location }),
      ...
      // OTP expired — register directly with phone as unique key
      const regRes = await fetch(`${BASE}/api/workers/register`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ name, skills: selectedSkill, location, contact: phone }),
      });

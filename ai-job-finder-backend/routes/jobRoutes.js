...
/* REPORT JOB — POST /api/jobs/report/:id */
router.post("/report/:id", async (req, res) => {
  try {
    const { reason, reporterPhone } = req.body;
    const job = await Job.findById(req.params.id);
    if (!job) return res.status(404).json({ message: "Job not found" });
    // In production: store in a Reports collection
    // For now: log it and return success
    console.log(`🚨 Job reported: ${req.params.id} | Reason: ${reason} | Reporter: ${reporterPhone}`);
    res.json({ success: true, message: "Report submitted. We will review this job." });
  } catch (err) {
    console.error(err);
